...
document.querySelectorAll('#board-popout').forEach(el => el.addEventListener('click', () => {settings.popoutWidth
  window.open(`../board-popout.html?seed=${bingoOpts.seed}&mode=${bingoOpts.mode}&version=${bingoOpts.version}${bingoOpts.lang === 'jp'? '&lang=jp' : ''}`, '_blank', `width=${settings.boardPopoutWidth}, height=${settings.boardPopoutHeight}, toolbar=no, location=no, directories=no, status=no, menubar=no, scrollbars=no, resizable=no, copyhistory=no`);
}));
