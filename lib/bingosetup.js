...
document.querySelectorAll('.board-popout').forEach(el => el.addEventListener('click', () => {settings.popoutWidth
  window.open(`../board-popout.html?seed=${bingoOpts.seed}&mode=${bingoOpts.mode}&version=${bingoList["info"].version}&lang=${bingoOpts.lang}`, '_blank', `width=605, height=570, toolbar=no, location=no, directories=no, status=no, menubar=no, scrollbars=no, resizable=no, copyhistory=no`);
}));
