...
let langParam = bingoOpts.lang === 'jp'? '&lang=jp' : ''

document.querySelectorAll('#board-popout').forEach(el => el.addEventListener('click', () => {settings.popoutWidth
  window.open(`${prefix}/board-popout.html?seed=${bingoOpts.seed}&mode=${bingoOpts.mode}&version=${bingoOpts.version}${langParam}`, '_blank', `width=${settings.boardPopoutWidth}, height=${settings.boardPopoutHeight}, toolbar=no, location=no, directories=no, status=no, menubar=no, scrollbars=no, resizable=no, copyhistory=no`);
}));

langParam = bingoOpts.lang === 'jp'? '?lang=jp' : ''
...
document.querySelectorAll('.popout').forEach(el => el.addEventListener('click', () => {
  ...
  window.open(`${prefix}/bingo-popout.html${langParam}#${encodeURIComponent(`${name}=${items.join(';;;')}`)}`, '_blank', `toolbar=no, location=no, directories=no, status=no, menubar=no, scrollbars=no, resizable=no, copyhistory=no, width=${settings.rowPopoutWidth}, height=${settings.rowPopoutHeight}`);
}));
