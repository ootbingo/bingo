...
const tokensComponent = ({ rows, icon }, matches) => {
  ...
  rows.forEach(icons => {
    ...
    icons.forEach(icon => {
      ...
      checkbox.innerHTML = `<input type="checkbox"><img src="./lib/item-tracker/icons/${filename}.ico">`;
...
const getDefaultMode = (goalName) => {
  for (name in window.trackerDefaults) {
    if (name.toLowerCase() === goalName.toLowerCase()) {
      return window.trackerDefaults[name]
    }
  }
}
...
document.querySelectorAll('td').forEach(el => {
  ...
  const mode = window.trackerOptions.settings[goal] || getDefaultMode(goal) || 'none';
  /*if (mode === 'none') {
    return;
  }*/
  ...
  for (let i = 0; i < window.trackerData.length; i++) {
    ...
    if ('regexJP' in matchingData) {
      ...
      if (matches)  {
        console.log(`jp match! (${matchingData.regex}) (${mode})`)
        console.log(matches)
        console.log()
        break;
      }
      ...
      console.log(goal)
      console.log(matchingData.regex)
      console.log(matchingData.regexJP)
    }
  }
