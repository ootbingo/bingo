...
window.trackerData = [
  ...
  },
  {
    ...
    regexJP: /ハートのかけら(\d{2}})個\(増殖禁止\)/, // not matched, is matched by (\d+).*HPs
    options: {
      counter: {
        icon: 'heartpiece'
      },
  ...
  },
  {
    ...
    regexJP: /Cアイテムのスロットを(\d+)箇所埋める/,
    options: {
      tokens: {
        rows: [
          ['stick', 'nut', 'bomb', 'fairybow', 'firearrows', 'dinsfire'],
          ['slingshot', 'ocarina', 'bombchu', 'hookshot', 'icearrows', 'faroreswind'],
          ['boomerang', 'well', 'bean', 'megatonhammer', 'lightarrows', 'nayruslove'],
