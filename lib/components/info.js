...
const template = ({ newSeed, version, lang }) => `
  ...
  ": HPs in Ice Cavern and Jabu-Jabu do not count"
    ...
    <ul>
      ...
      <li>
        ${localize("When a goal may be considered done:")}<br />
        ${localize("Item: When you are holding it over head")}<br />
        ${localize(
      ...
      <li>
        ${localize("Goal completion clarifications:")}<br />
        ${localize("Cow in House: When the final textbox after finishing the obstacle course in under 00:50 completes")}<br />
        ${localize("Defeat a White Wolfos: If in Gerudo Training Grounds, all Wolfos enemies do not need to be defeated")}<br />
        ${localize('Double Magic/Defense: When the text "Your Magic Meter / defensive power is enhanced!" appears')}<br />
        ${localize("X Hearts: When the final Heart Piece / Heart Container is overhead")}<br />
      </li>
    </ul>
    ...
    <ul>
      ...
      <li>${localize("On approved N64 emulators: Blank A for Bottom of the Well Bombchus")}
    </ul>
